...
class RollingFeatureExtractor:
    ...
    def _entropy(values: Iterable[str]) -> float:
        ...
        total = len(values)
        return float(-sum((n / total) * log2(n / total) for n in counts.values()))
    ...
    def fit_time_statistics(self, timestamps: pd.Series) -> None:
        hours = pd.to_datetime(timestamps, utc=True).dt.hour + pd.to_datetime(
            timestamps, utc=True
        ).dt.minute / 60.0
    ...
    def transform_event(self, event: dict) -> dict:
        timestamp = pd.Timestamp(event["timestamp"], tz="UTC") if not isinstance(
            event["timestamp"], pd.Timestamp
        ...
        source_history.append((timestamp.timestamp(), event))
        self.global_events.append((timestamp.timestamp(), event))
        ...
        auth_events = sum(1 for e in source_events if int(e.get("failed_logins", 0)) > 0)
        ...
        if previous:
            distance = self._distance(
                previous[0], previous[1], event.get("latitude"), event.get("longitude")
            )
        ...
        return {
            ...
            "failed_login_rate": failed / max(auth_events + 1, 1),
    ...
    def transform(self, df: pd.DataFrame, reset: bool = True) -> pd.DataFrame:
        ...
        features = [self.transform_event(row.to_dict()) for _, row in frame.iterrows()]
